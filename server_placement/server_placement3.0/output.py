...
def Calculate_Reward(a, uti, disconnect_rate, Delay_Outage_Rate):
    ...
    r = 6*reward_utilization-cost_open-1000*disconnect_rate-1000*Delay_Outage_Rate
    return r
...
class AdaptSpeed:
    ...
    def Calculate_Reward_in_Other_Action(self, env, other_action, a_last, i_episode, t, Date, SLOTNUM):
        ...
        disconnect_rate, Avg_Delay, Delay_Outage_Rate = Calculate_disconnect_outofdelay(temp_env)
        temp_r = Calculate_Reward(other_action, temp_s_, disconnect_rate, Delay_Outage_Rate)
        del temp_env
        gc.collect()
...
class ConvergenceRate:
    ...
    def store_reward(self, DRL, t, i_episode, Date, path):
        ...
        self.a = aa[0,:].numpy()
        self.s_ = self.env.update_only(self.a, self.a_last, i_episode, t+(Date-1001)*self.SLOTNUM)
        r = Calculate_Reward(self.env, self.a, self.s_)
        self.reward_t[t] = r
        self.s_ = np.transpose(self.s_)
        self.s = self.s_
        self.a_last = self.a
